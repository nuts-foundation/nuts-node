...
::
      ...
      --crypto.storage string                         Storage to use, 'external' for an external backend (recommended), 'fs' for file system (for development purposes), 'vaultkv' for Vault KV store (will be replaced by external backend in future). (default "fs")
