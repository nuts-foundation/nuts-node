...
::
      ...
      --storage.session.type string                       Type of session storage to use. Options are: redis, sql, memory. Note that memory storage does not survive restarts and does not support multiple nodes. If using Redis, storage.session.redis also needs to be configured. (default "memory")
