...
## Security
* #4456: Fix a crash when logging a Verifiable Credential/Presentation without an ``id`` (optional per the VC/VP data model). Upgrading ``sirupsen/logrus`` to v1.10.0 changed how it formats log fields: it now calls a field's ``String()`` method directly, and ``*ssi.URI``'s ``String()`` has a value receiver, so passing a nil ID panicked the process instead of just logging it oddly. By @reinkrul in https://github.com/nuts-foundation/nuts-node/pull/4456
