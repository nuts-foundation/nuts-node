...
var authorizerDID = *vdr.TestDIDB
var requesterDID = *vdr.TestDIDA
var requesterDIDDocument = getRequesterDIDDocument()
...
func getAuthorizerSigningKey() *ssi.URI {
	serviceID, _ := ssi.ParseURI(authorizerDID.String() + "#signing-key")

	return serviceID
}

func getRequesterSigningKey() *ssi.URI {
	keyID, _ := ssi.ParseURI(requesterDID.String() + "#signing-key")

	return keyID
}

func getRequesterDIDDocument() *did.Document {
	id := requesterDID
	...
	key, err := did.NewVerificationMethod(signingKeyID, ssi.JsonWebKey2020, id, requesterSigningKey.Public())
	if err != nil {
		panic(err)
	}
...
func TestAuth_CreateAccessToken(t *testing.T) {
	...
	t.Run("broken identity token", func(t *testing.T) {
		ctx := createContext(t)
		defer ctx.ctrl.Finish()
		ctx.nameResolver.EXPECT().Get(concept.OrganizationConcept, false, authorizerDID.String()).MinTimes(1).Return(orgConceptName, nil)
	...
	t.Run("valid - without user identity", func(t *testing.T) {
		...
		ctx.keyResolver.EXPECT().ResolveSigningKeyID(requesterDID, gomock.Any()).MinTimes(1).Return(requesterSigningKeyID.String(), nil)
		ctx.nameResolver.EXPECT().Get(concept.OrganizationConcept, false, authorizerDID.String()).MinTimes(1).Return(orgConceptName, nil)
		ctx.didResolver.EXPECT().Resolve(requesterDID, gomock.Any()).Return(getRequesterDIDDocument(), nil, nil).AnyTimes()
		ctx.serviceResolver.EXPECT().GetCompoundServiceEndpoint(requesterDID, expectedService, services.OAuthEndpointType, true).Return(expectedAudience, nil)
		ctx.privateKeyStore.EXPECT().Exists(requesterSigningKeyID.String()).Return(true)
		ctx.privateKeyStore.EXPECT().SignJWT(gomock.Any(), requesterSigningKeyID.String()).Return("expectedAccessToken", nil)
	...
	t.Run("valid - all fields", func(t *testing.T) {
		...
		ctx.serviceResolver.EXPECT().GetCompoundServiceEndpoint(requesterDID, expectedService, services.OAuthEndpointType, true).Return(expectedAudience, nil)
		ctx.privateKeyStore.EXPECT().Exists(requesterSigningKeyID.String()).Return(true)
		ctx.privateKeyStore.EXPECT().SignJWT(gomock.Any(), requesterSigningKeyID.String()).Return("expectedAT", nil)
...
func TestService_validateIssuer(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		...
		tokenCtx := validContext()
		ctx.keyResolver.EXPECT().ResolveSigningKey(authorizerSigningKeyID.String(), gomock.Any()).MinTimes(1).Return(authorizerSigningKey.Public(), nil)
		ctx.nameResolver.EXPECT().Get(concept.OrganizationConcept, false, authorizerDID.String()).Return(orgConceptName, nil)

		err := ctx.oauthService.validateIssuer(tokenCtx)
		assert.NoError(t, err)
		assert.Equal(t, "Carebears", tokenCtx.authorizerName)
	})
	...
	t.Run("unable to resolve name", func(t *testing.T) {
		...
		tokenCtx := validContext()
		ctx.keyResolver.EXPECT().ResolveSigningKey(authorizerSigningKeyID.String(), gomock.Any()).MinTimes(1).Return(authorizerSigningKey.Public(), nil)
		ctx.nameResolver.EXPECT().Get(concept.OrganizationConcept, false, authorizerDID.String()).Return(nil, errors.New("failed"))
	...
	t.Run("unable to resolve name from credential", func(t *testing.T) {
		...
		tokenCtx := validContext()
		ctx.keyResolver.EXPECT().ResolveSigningKey(authorizerSigningKeyID.String(), gomock.Any()).Return(authorizerSigningKey.Public(), nil)
		ctx.nameResolver.EXPECT().Get(concept.OrganizationConcept, false, authorizerDID.String()).Return(falseConceptName, nil)

		err := ctx.oauthService.validateIssuer(tokenCtx)
		assert.EqualError(t, err, "invalid jwt.issuer: authorizer has invalid organization VC: no value for given path")
	})
	t.Run("unable to resolve key", func(t *testing.T) {
		...
		ctx.keyResolver.EXPECT().ResolveSigningKey(authorizerSigningKeyID.String(), gomock.Any()).MinTimes(1).Return(nil, fmt.Errorf("not found"))
...
func TestService_validateSubject(t *testing.T) {
	t.Run("subject managed", func(t *testing.T) {
		...
		tokenCtx := validContext()
		tokenCtx.jwtBearerToken.Set(jwt.SubjectKey, requesterDID.String())
...
func TestService_validateAud(t *testing.T) {
	...
	t.Run("error - endpoint resolve returns error", func(t *testing.T) {
		...
		defer ctx.ctrl.Finish()
		tokenCtx := validContext()
		ctx.serviceResolver.EXPECT().GetCompoundServiceEndpoint(requesterDID, expectedService, services.OAuthEndpointType, true).Return("", types.ErrNotFound)
...
func TestService_parseAndValidateJwtBearerToken(t *testing.T) {
	...
	t.Run("valid token", func(t *testing.T) {
		...
		ctx.keyResolver.EXPECT().ResolveSigningKey(authorizerSigningKeyID.String(), gomock.Any()).Return(authorizerSigningKey.PublicKey, nil)

		err := ctx.oauthService.parseAndValidateJwtBearerToken(tokenCtx)
		assert.NoError(t, err)
		assert.Equal(t, authorizerDID.String(), tokenCtx.jwtBearerToken.Issuer())
		assert.Equal(t, authorizerSigningKeyID.String(), tokenCtx.kid)
	})
}
...
func TestService_buildAccessToken(t *testing.T) {
	...
	t.Run("build an access token", func(t *testing.T) {
		...
		assert.Equal(t, requesterDID.String(), recordedClaims["iss"])
...
func TestService_CreateJwtBearerToken(t *testing.T) {
	...
	t.Run("create a JwtBearerToken", func(t *testing.T) {
		...
		ctx.didResolver.EXPECT().Resolve(requesterDID, gomock.Any()).Return(requesterDIDDocument, nil, nil).AnyTimes()
		ctx.serviceResolver.EXPECT().GetCompoundServiceEndpoint(requesterDID, expectedService, services.OAuthEndpointType, true).Return(expectedAudience, nil)
		ctx.keyResolver.EXPECT().ResolveSigningKeyID(authorizerDID, gomock.Any()).MinTimes(1).Return(authorizerSigningKeyID.String(), nil)
		ctx.privateKeyStore.EXPECT().SignJWT(gomock.Any(), authorizerSigningKeyID.String()).Return("token", nil)
	...
	t.Run("requester without endpoint", func(t *testing.T) {
		ctx := createContext(t)
		document := getRequesterDIDDocument()
		document.Service = []did.Service{}

		ctx.serviceResolver.EXPECT().GetCompoundServiceEndpoint(requesterDID, expectedService, services.OAuthEndpointType, true).Return("", didman.ErrServiceNotFound)
	...
	t.Run("request without requester", func(t *testing.T) {
		ctx := createContext(t)
	...
	t.Run("signing error", func(t *testing.T) {
		...
		ctx.serviceResolver.EXPECT().GetCompoundServiceEndpoint(requesterDID, expectedService, services.OAuthEndpointType, true).Return(expectedAudience, nil)
		ctx.keyResolver.EXPECT().ResolveSigningKeyID(authorizerDID, gomock.Any()).MinTimes(1).Return(authorizerSigningKeyID.String(), nil)
		ctx.privateKeyStore.EXPECT().SignJWT(gomock.Any(), authorizerSigningKeyID.String()).Return("", errors.New("boom!"))
...
func Test_claimsFromRequest(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		assert.Equal(t, request.Authorizer, claims[jwt.IssuerKey])
		assert.Equal(t, 0, claims[jwt.NotBeforeKey])
		assert.Equal(t, request.Requester, claims[jwt.SubjectKey])
...
func validContext() *validationContext {
	...
	return &validationContext{
		...
		kid:            authorizerSigningKeyID.String(),
...
func signToken(context *validationContext) {
	hdrs := jws.NewHeaders()
	err := hdrs.Set(jws.KeyIDKey, authorizerSigningKeyID.String())
	if err != nil {
		panic(err)
	}
	signedToken, err := jwt.Sign(context.jwtBearerToken, jwa.ES256, authorizerSigningKey, jwt.WithHeaders(hdrs))
	if err != nil {
		panic(err)
	}
