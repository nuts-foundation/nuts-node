...
const errInvalidOrganizationVC = "authorizer has invalid organization VC: %w"
...
func (s *service) CreateAccessToken(request services.CreateAccessTokenRequest) (*services.AccessTokenResult, error) {
	...
	// check the authorizer against the registry, according to RFC003 §5.2.1.3
	// checks signing certificate and sets vendor, authorizerName in validationContext
	if err := s.validateIssuer(&context); err != nil {
		return nil, err
	}

	// check if the requester is registered by this vendor, according to RFC003 §5.2.1.8
	...
	if usi != nil {
		...
		if err := s.validateAuthorizer(&context); err != nil {
			return nil, err
		}
	}
...
// checks if the name from the login contract matches with the registered name of the issuer.
func (s *service) validateAuthorizer(context *validationContext) error {
	if context.contractVerificationResult.ContractAttributes[contract.LegalEntityAttr] != context.authorizerName || context.contractVerificationResult.ContractAttributes[contract.LegalEntityCityAttr] != context.authorizerCity {
...
// - the authorizer name/city which must match the login contract.
func (s *service) validateIssuer(context *validationContext) error {
	...
	if context.authorizerName, err = orgConcept.GetString(concept.OrganizationName); err != nil {
		return fmt.Errorf(errInvalidIssuerFmt, fmt.Errorf(errInvalidOrganizationVC, err))
	}
	if context.authorizerCity, err = orgConcept.GetString(concept.OrganizationCity); err != nil {
		return fmt.Errorf(errInvalidIssuerFmt, fmt.Errorf(errInvalidOrganizationVC, err))
	}
...
// GetOAuthEndpointURL returns the oauth2 endpoint URL of the requester for a service
func (s *service) GetOAuthEndpointURL(service string, requester did.DID) (url.URL, error) {
	endpointURL, err := s.serviceResolver.GetCompoundServiceEndpoint(requester, service, services.OAuthEndpointType, true)
	if err != nil {
...
func (s *service) CreateJwtGrant(request services.CreateJwtGrantRequest) (*services.JwtBearerTokenResult, error) {
	...
	endpointURL, err := s.serviceResolver.GetCompoundServiceEndpoint(*requester, request.Service, services.OAuthEndpointType, true)
	if err != nil {
		return nil, err
	}
	...
	now := time.Now()
	signingKeyID, err := s.keyResolver.ResolveSigningKeyID(*authorizer, &now)
	if err != nil {
		return nil, err
	}
...
func claimsFromRequest(request services.CreateJwtGrantRequest, audience string) map[string]interface{} {
	...
	result[jwt.IssuerKey] = request.Authorizer
	result[jwt.NotBeforeKey] = 0
	result[jwt.SubjectKey] = request.Requester
...
// The token gets signed with the requesters private key and returned as a string.
