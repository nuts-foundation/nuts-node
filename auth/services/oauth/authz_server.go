...
func (s *authzServer) validateAccessTokenRequest(ctx context.Context, bearerToken string) (*validationContext, error) {
	...
	expiration, _ := validationCtx.jwtBearerToken.Expiration()
	issuedAt, _ := validationCtx.jwtBearerToken.IssuedAt()
	if expiration.Sub(issuedAt).Seconds() > BearerTokenMaxValidity {
