...
func (cs *StatusList2021) Credential(ctx context.Context, issuerDID did.DID, page int) (*vc.VerifiableCredential, error) {
	statusListCredentialURL := toStatusListCredential(cs.baseURL, issuerDID, page)
	...
	for {
		err := cs.db.Transaction(func(tx *gorm.DB) error {
			...
			if credentialIssuer.LastIssuedIndex > maxBitstringIndex {
				...
				credentialIssuer.SubjectID = toStatusListCredential(cs.baseURL, issuer, credentialIssuer.Page)
...
func toStatusListCredential(baseURL *url.URL, issuer did.DID, page int) string {
	// https://example.com/statuslist/<did>/page
	return baseURL.JoinPath("statuslist", issuer.String(), strconv.Itoa(page)).String()
}
