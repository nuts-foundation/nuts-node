...
func TestStatusList2021_Entry(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		t.Run("new issuer", func(t *testing.T) {
			statusListCredential, _ := s.credentialURLFunc(aliceDID, 1)
			// confirm empty DB
		...
		t.Run("credential rollover", func(t *testing.T) {
			...
			statusListCredential, _ = s.credentialURLFunc(aliceDID, 2) // now expect page 2 to be used
...
func TestStatusList2021_Credential(t *testing.T) {
	...
	t.Run("ok - loadCredential failed", func(t *testing.T) {
		...
		subjectID, err := s.credentialURLFunc(bobDID, 0)
		require.NoError(t, err)
...
func TestStatusList2021_buildAndSignVC(t *testing.T) {
	...
	subjectID, err := cs.credentialURLFunc(aliceDID, 1)
	require.NoError(t, err)
	encodedList, err := compress(*newBitstring())
	require.NoError(t, err)
