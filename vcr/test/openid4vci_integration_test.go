...
// TestOpenID4VCIHappyFlow tests issuing a VC using OIDC4VCI.
...
// TestOpenID4VCI_Metadata tests resolving OIDC4VCI metadata, while the party hasn't registered its base URL.
...
// TestOpenID4VCIDisabled tests the issuer won't try to issue over OIDC4VCI when it's disabled.
func TestOpenID4VCIDisabled(t *testing.T) {
	...
	t.Run("API returns 404", func(t *testing.T) {
		resp, err := http.Get(core.JoinURLPaths(baseURL, "n2n", "identity", url.PathEscape(walletDID.String()), oidc4vci.WalletMetadataWellKnownPath))
		require.NoError(t, err)
...
// TestOpenID4VCIErrorResponses tests the API returns the correct error responses (as specified in the OIDC4VCI spec, not as Problem types).
