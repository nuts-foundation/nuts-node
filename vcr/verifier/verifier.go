...
func (v verifier) VerifyVP(vp vc.VerifiablePresentation, verifyVCs bool, verifyVCsTrusted bool, validAt *time.Time) ([]vc.VerifiableCredential, error) {
	return v.doVerifyVP(&v, vp, verifyVCs, verifyVCsTrusted, validAt)
}

// doVerifyVP delegates VC verification to the supplied Verifier, to aid unit testing.
func (v verifier) doVerifyVP(vcVerifier Verifier, vp vc.VerifiablePresentation, verifyVCs bool, verifyVCsTrusted bool, validAt *time.Time) ([]vc.VerifiableCredential, error) {
	...
	if verifyVCs {
		for _, current := range vp.VerifiableCredential {
			err := vcVerifier.Verify(current, !verifyVCsTrusted, true, validAt)
			if err != nil {
