    ...
    "type": [
      ...
      "VerifiableCredential"
		...
		t.Run("error - invalid proof", func(t *testing.T) {
			...
			assert.EqualError(t, err, "verification error: invalid LD-proof for presentation: json: cannot unmarshal string into Go value of type proof.LDProof")
			assert.Empty(t, vcs)
		})
		t.Run("error - no proof", func(t *testing.T) {
			...
			assert.EqualError(t, err, "verification error: presentation should have exactly 1 proof, got 0")
			assert.Empty(t, vcs)
		})
	})
}
