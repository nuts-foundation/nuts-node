...
for x in $(seq 100); do
	...
	docker logs denylist-nuts-node-defaults-1 2>&1 | tail -n1 | grep 'Validated certificate'
	if [ $? -ne 0 ]; then
...
for x in $(seq 100); do
	# Connect to the nuts-node with a valid client certificate
	#curl --fail --cert client-allowed.crt --key client-allowed.key "https://localhost:1323/status/diagnostics"
	openssl s_client -connect localhost:25555 -cert client-allowed.crt -key client-allowed.key -CAfile truststore-development.pem -verify_return_error -tls1_2 < <(echo "Hello Nuts 🥜")
	if [ $? -ne 0 ]; then
	...
	# Check the logs have the right contents
	docker logs denylist-nuts-node-github-1 2>&1 | tail -n1 | grep 'Validated certificate'
	if [ $? -ne 0 ]; then
...
echo "------------------------------------"
echo "Connecting (defaults) 100x with blocked cert (tls v1.2).."
echo "------------------------------------"
for x in $(seq 100); do
	...
	# Check the logs have the right contents
	docker logs denylist-nuts-node-defaults-1 2>&1 | tail -n1 | grep 'Rejecting banned certificate'
	if [ $? -ne 0 ]; then
...
echo "------------------------------------"
echo "Connecting (github) 100x with blocked cert (tls v1.2).."
echo "------------------------------------"
for x in $(seq 100); do
	# Connect to the nuts-node with a blocked client certificate
	openssl s_client -connect localhost:25555 -cert client-blocked.crt -key client-blocked.key -CAfile truststore-development.pem -verify_return_error -tls1_2 < <(echo "Hello Nuts 🥜")
	if [ $? -eq 0 ]; then
	...
	# Check the logs have the right contents
	docker logs denylist-nuts-node-github-1 2>&1 | tail -n1 | grep 'Rejecting banned certificate'
	if [ $? -ne 0 ]; then
...
echo "------------------------------------"
echo "Connecting (defaults) 100x with blocked cert (tls v1.3).."
echo "------------------------------------"
for x in $(seq 100); do
	# Connect to the nuts-node with a blocked client certificate
...
echo "------------------------------------"
echo "Connecting (github) 100x with blocked cert (tls v1.3).."
echo "------------------------------------"
for x in $(seq 100); do
	# Connect to the nuts-node with a blocked client certificate
	openssl s_client -connect localhost:25555 -cert client-blocked.crt -key client-blocked.key -CAfile truststore-development.pem -verify_return_error -tls1_3 < <(echo "Hello Nuts 🥜")
	# Ignore exit code from openssl with tls v1.3 because it is unreliable.
	# Depend entirely on the log check below instead.
