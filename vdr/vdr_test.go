...
func TestVDR_ConflictingDocuments(t *testing.T) {
	...
	t.Run("diagnostics", func(t *testing.T) {
		...
		t.Run("ok - 1 owned conflict", func(t *testing.T) {
			...
			keyID.Fragment = "1"
			_, _, _ = client.New(audit.TestContext(), nutsCrypto.StringNamingFunc(keyID.String()), orm.AssertionKeyUsage())
			ctrl := gomock.NewController(t)
			pkiMock := pki.NewMockValidator(ctrl)
		...
		t.Run("ok - 1 owned conflict in controlled document", func(t *testing.T) {
			...
			_, keyVendor, _ := test.keyStore.New(audit.TestContext(), nutsCrypto.StringNamingFunc("did:nuts:vendor#keyVendor-1"), orm.AssertionKeyUsage())
			...
			// organization
			_, keyOrg, _ := test.keyStore.New(audit.TestContext(), nutsCrypto.StringNamingFunc("did:nuts:org#keyOrg-1"), orm.AssertionKeyUsage())
...
func TestVDR_Migrate(t *testing.T) {
	...
	t.Run("controller migration", func(t *testing.T) {
		...
		t.Run("makes documents self-controlled", func(t *testing.T) {
			...
			keyRef, publicKey, err := keyStore.New(ctx.ctx, didnuts.DIDKIDNamingFunc, orm.AssertionKeyUsage())
			require.NoError(t, err)
