...
func TestVDRIntegration_Test(t *testing.T) {
	...
	docA, _, err := ctx.vdr.Create(dochelper.DefaultCreationOptions())
	...
	docB, _, err := ctx.vdr.Create(dochelper.DefaultCreationOptions())
	...
	docUpdater := &dochelper.Manipulator{KeyCreator: ctx.cryptoInstance, Updater: *ctx.vdr, Resolver: ctx.docResolver}
...
func TestVDRIntegration_ConcurrencyTest(t *testing.T) {
	...
	// Start with a first and fresh document named DocumentA.
	initialDoc, _, err := ctx.vdr.Create(dochelper.DefaultCreationOptions())
	require.NoError(t, err)
...
func TestVDRIntegration_ReprocessEvents(t *testing.T) {
	...
	didDoc, key, _ := ctx.docCreator.Create(dochelper.DefaultCreationOptions())
...
type testContext struct {
	...
	docCreator     dochelper.Creator
	docResolver    dochelper.Resolver
...
func setup(t *testing.T) testContext {
	...
	docResolver := dochelper.Resolver{Store: didStore}
	docFinder := dochelper.Finder{Store: didStore}
	docCreator := dochelper.Creator{KeyStore: cryptoInstance}
	...
	nutsNetwork := network.NewNetworkInstance(
		networkCfg,
		dochelper.KeyResolver{Store: didStore},
		cryptoInstance,
		cryptoInstance,
		docResolver,
		docFinder,
