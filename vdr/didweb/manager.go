...
func (m Manager) GenerateDocument(ctx context.Context, keyFlags didsubject.DIDKeyFlags) (*didsubject.DIDDocument, error) {
	...
	for _, keyType := range keyTypes {
		if keyType.Is(keyFlags) {
			verificationMethod, err := m.GenerateVerificationMethod(ctx, *newDID, keyType)
			if err != nil {
				return nil, err
			}
...
func (m Manager) GenerateVerificationMethod(ctx context.Context, controller did.DID, keyUsage didsubject.DIDKeyFlags) (*did.VerificationMethod, error) {
	var err error
	...
	if keyUsage.Is(didsubject.KeyAgreementUsage) {
		...
		// todo requires update to crypto module
