...
type DIDControllerResolver interface {
...
type KeyResolver interface {
	...
	// ResolveKey looks for a valid key of the given RelationType for the give DID, and returns its ID and the key itself.
	// If multiple keys are valid, the first one is returned.
	// An ErrKeyNotFound is returned when no key (of the specified type) is found.
