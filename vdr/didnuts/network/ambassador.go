...
func NewAmbassador(networkClient network.Transactions, store store.Store, eventManager events.Event) Ambassador {
	resolver := didnuts.Resolver{Store: store}
	return &ambassador{
		networkClient: networkClient,
		store:         store,
		keyResolver:   didnuts.NutsKeyResolver{Resolver: resolver},
		docResolver:   didnuts.Resolver{Store: store},
...
func (n *ambassador) callback(tx dag.Transaction, payload []byte) error {
	...
	if err := service.NetworkDocumentValidator().Validate(nextDIDDocument); err != nil {
...
func (n *ambassador) handleCreateDIDDocument(transaction dag.Transaction, proposedDIDDocument did.Document) error {
	...
	err = n.store.Add(proposedDIDDocument, store.Transaction{
