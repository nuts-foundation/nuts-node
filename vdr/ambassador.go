...
func (n *ambassador) handleUpdateDIDDocument(transaction dag.Transaction, proposedDIDDocument did.Document) error {
	...
	var currentDIDMeta *types.DocumentMetadata
	var err error
	for _, ref := range transaction.Previous() {
		currentDIDDocument, currentDIDMeta, err = n.didStore.Resolve(proposedDIDDocument.ID, &types.ResolveMetadata{AllowDeactivated: true, SourceTransaction: &ref})
		if err != nil && !errors.Is(err, types.ErrNotFound) {
