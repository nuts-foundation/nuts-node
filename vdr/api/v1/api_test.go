...
func TestWrapper_AddNewVerificationMethod(t *testing.T) {
	...
	t.Run("ok - without key usage", func(t *testing.T) {
		ctx := newMockContext(t)
		ctx.docUpdater.EXPECT().AddVerificationMethod(*did123, dochelper.DefaultCreationOptions().KeyFlags).Return(newMethod, nil)
	...
	t.Run("ok - with key usage", func(t *testing.T) {
		...
		expectedKeyUsage := dochelper.DefaultCreationOptions().KeyFlags | types.AuthenticationUsage | types.CapabilityDelegationUsage
