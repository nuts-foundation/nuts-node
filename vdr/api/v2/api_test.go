...
func TestWrapper_CreateDID(t *testing.T) {
	t.Run("ok - defaults", func(t *testing.T) {
		...
		ctx.subjectManager.EXPECT().Create(gomock.Any(), management.EmptyCreationOptions().With(management.MethodCreationOption{Method: didweb.MethodName})).Return([]did.Document{didDoc}, "subject", nil)

		response, err := ctx.client.CreateDID(nil, CreateDIDRequestObject{Body: &CreateDIDJSONRequestBody{}})

		require.NoError(t, err)
		assert.Len(t, response.(CreateDID200JSONResponse), 1)
	})
...
type mockContext struct {
	...
	vdr            *vdr.MockVDR
	subjectManager *management.MockSubjectManager
...
func newMockContext(t *testing.T) mockContext {
	...
	subjectManager := management.NewMockSubjectManager(ctrl)
