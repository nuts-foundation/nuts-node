...
func (m managedServiceValidator) resolveOrReturnEndpoint(service did.Service, cache map[string]*did.Document) (any, error) {
	...
	if IsServiceReference(serviceEndpoint) {
		serviceURI, err := ssi.ParseURI(serviceEndpoint)
		if err != nil {
			return nil, err
		}
		if err = ValidateServiceReference(*serviceURI); err != nil {
			return nil, err
		}
		resolvedService, err := m.serviceResolver.ResolveEx(*serviceURI, 0, DefaultMaxServiceReferenceDepth, cache)
		if err != nil {
			return nil, err
		}
