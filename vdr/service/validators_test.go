		...
		table := []validatorTest{
			...
			{"ok - self reference", func() did.Document {
				...
				didDoc.Service = append(didDoc.Service, did.Service{
					ID:              ssi.URI{},
					Type:            "self_reference",
					ServiceEndpoint: MakeServiceReference(didDoc.ID, didDoc.Service[0].Type),
				})
			...
			{"nok - resolve fails", func() did.Document {
				didDoc := newDidDoc()
				didDoc.Service[0].ServiceEndpoint = MakeServiceReference(referencedDocument.ID, "does_not_exist")
		...
		table := []validatorTest{
			...
			{"ok - validates after resolving", func() did.Document {
				...
				didDoc.Service[0] = did.Service{
					...
					ServiceEndpoint: MakeServiceReference(didDoc.ID, "otherService"),
				}
				didDoc.Service = append(didDoc.Service, did.Service{
					Type:            "otherService",
...
func newDidDoc() did.Document {
	id := did.MustParseDID("did:nuts:8NYzfsndZJHh6GqzKiSBpyERrFxuX64z6tE5raa7nEjm")
	document := did.Document{
		ID: id,
		Service: []did.Service{
			{
				ID:              ssi.MustParseURI(id.String() + "#1234"),
				Type:            "test",
	...
	err := vm.UnmarshalJSON([]byte(`{
      "id": "did:nuts:8NYzfsndZJHh6GqzKiSBpyERrFxuX64z6tE5raa7nEjm#bYcuet6EHojMlaMqwNoC3c6etKlUHoJ9rRvUu3ZKEEw",
      "type": "JsonWebKey2020",
      "controller": "did:nuts:8NYzfsndZJHh6GqzKiSBpyERrFxuX64z6tE5raa7nEjm",
      "publicKeyJwk": {
        "crv": "P-256",
        "kty": "EC",
        "x": "Mw2BMOqhgrt8kA6b02e6h8edqSFMkiZybR3mi3HIbgE",
        "y": "aXCRfnYK74NJrSWBG7u-iOFglKokIsRr38bmR9Xaftc"
      }
    }`))
	if err != nil {
		panic(err)
