...
func TestVDRIntegration_Test(t *testing.T) {
	...
	docAs, _, err := ctx.vdr.Create(ctx.audit, management.EmptyCreationOptions())
	require.NoError(t, err)
	assert.Len(t, docAs, 2)
	...
	docBs, _, err := ctx.vdr.Create(ctx.audit, management.EmptyCreationOptions())
	require.NoError(t, err, "unexpected error while creating DocumentB")
	require.Len(t, docBs, 2)
	docB := &docBs[0]
...
func TestVDRIntegration_ReprocessEvents(t *testing.T) {
	...
	didDoc, key, _ := ctx.vdr.NutsDocumentManager().Create(audit.TestContext(), management.EmptyCreationOptions())
