...
func FlagSet() *pflag.FlagSet {
	...
	flags.String("http.client.log", string(defs.Client.Log), fmt.Sprintf("What to log about outgoing HTTP requests made by the node. Options are '%s', '%s' (log request method, URI and response code), and '%s' (log the request and response body, in addition to the metadata). When debug vebosity is set the request and response headers are also logged.", http.LogNothingLevel, http.LogMetadataLevel, http.LogMetadataAndBodyLevel))
