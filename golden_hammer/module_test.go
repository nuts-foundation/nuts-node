...
func TestGoldenHammer_Fix(t *testing.T) {
	...
	t.Run("DID methods other than did:nuts are ignored", func(t *testing.T) {
		...
		ctx.documentOwner.EXPECT().ListOwned(gomock.Any()).Return([]did.DID{otherDID}, nil)
		service := ctx.hammer
	...
	t.Run("nothing to fix", func(t *testing.T) {
		...
		ctx.documentOwner.EXPECT().ListOwned(gomock.Any()).Return([]did.DID{vendorDID, clientADID, clientBDID}, nil)
	...
	t.Run("to be registered on vendor DID and client DIDs", func(t *testing.T) {
		...
		ctx.documentOwner.EXPECT().ListOwned(gomock.Any()).Return([]did.DID{clientADID, vendorDID, clientBDID}, nil)
		gomock.InOrder(
	...
	t.Run("vendor identifier can't be resolved from TLS", func(t *testing.T) {
		...
		ctx.documentOwner.EXPECT().ListOwned(gomock.Any()).Return([]did.DID{vendorDID}, nil)
		service := ctx.hammer
	...
	t.Run("to be registered on client DIDs", func(t *testing.T) {
		...
		ctx.documentOwner.EXPECT().ListOwned(gomock.Any()).Return([]did.DID{vendorDID, clientADID, clientBDID, clientCDID}, nil)
...
func TestGoldenHammer_Lifecycle(t *testing.T) {
	t.Run("enabled", func(t *testing.T) {
		...
		ctx := newMockContext(t)
		ctx.documentOwner.EXPECT().ListOwned(gomock.Any()).DoAndReturn(func(_ context.Context) ([]did.DID, error) {
			fixCalled.Add(1)
			return []did.DID{}, nil
		}).MinTimes(1)
		service := ctx.hammer
