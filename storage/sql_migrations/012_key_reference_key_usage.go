...
//     statement backfills existing rows to 0. That's not a real DIDKeyFlags value - every actual
//     key supports at least AssertionKeyUsage (0x0F) - so crypto.Crypto.Migrate() can use "still at
//     0" to unambiguously mean "not yet corrected to the currently configured backend's real
//     usage", without ever mistaking an already-corrected row for one that still needs correcting.
...
func keyReferenceKeyUsage012Statements(dbType string) []string {
	const addColumn = "alter table key_reference add column key_usage SMALLINT not null default 0"
	switch dbType {
	case "postgres", "mysql":
		return []string{addColumn, "alter table key_reference alter column key_usage drop default"}
	case "sqlserver", "azuresql":
		return []string{
			"alter table key_reference add key_usage SMALLINT not null constraint df_key_reference_key_usage default 0",
...
// crypto.Crypto.Migrate() sets the real value for existing rows this migration backfills to 0,
// since only it knows the currently configured backend.
