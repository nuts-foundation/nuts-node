...
func Test_redisDatabase_createStore(t *testing.T) {
	...
	t.Run("Redis Sentinel", func(t *testing.T) {
		...
		t.Run("error - missing master name param", func(t *testing.T) {
			_, err := createRedisDatabase(RedisConfig{
				Address: "rediss://instance1:1234,instance2:4321?sentinelUsername=sentinel-user",
			})

			assert.ErrorContains(t, err, "Redis Sentinel options specified in connection string, but missing required option: sentinelMasterName")
		})
