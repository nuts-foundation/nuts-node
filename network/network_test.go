...
import (
	...
	testPKI "github.com/nuts-foundation/nuts-node/v6/test/pki"
...
func TestNetwork_Configure(t *testing.T) {
	...
	t.Run("ok - configured node DID", func(t *testing.T) {
		...
		err := test.network.Configure(core.TestServerConfig(func(config *core.ServerConfig) {
	...
	t.Run("unable to create datadir", func(t *testing.T) {
		...
		err := ctx.network.Configure(core.TestServerConfig(func(config *core.ServerConfig) {
			config.Datadir = "network_test.go"
		}))
		assert.Error(t, err)
	})
...
func TestNetwork_CreateTransaction(t *testing.T) {
	...
	t.Run("ok - attach key", func(t *testing.T) {
		...
		err := cxt.start()
		require.NoError(t, err)
		_, key, _ := cxt.keyStore.New(audit.TestContext(), crypto.StringNamingFunc("signing-key"), orm.AssertionKeyUsage())
	...
	t.Run("ok - detached key", func(t *testing.T) {
		...
		err := cxt.start()
		require.NoError(t, err)
		_, _, _ = cxt.keyStore.New(audit.TestContext(), crypto.StringNamingFunc("signing-key"), orm.AssertionKeyUsage())
		cxt.state.EXPECT().Head(gomock.Any())
		cxt.state.EXPECT().Add(gomock.Any(), gomock.Any(), payload)
	...
	t.Run("private transaction", func(t *testing.T) {
		...
		t.Run("ok", func(t *testing.T) {
			...
			err := cxt.start()
			require.NoError(t, err)
			_, _, _ = cxt.keyStore.New(audit.TestContext(), crypto.StringNamingFunc("did:nuts:sender#signing-key"), orm.AssertionKeyUsage())
			...
			_, err = cxt.network.CreateTransaction(ctx, TransactionTemplate(payloadType, payload, "did:nuts:sender#signing-key").WithPrivate([]did.DID{*sender, *receiver}))
			assert.NoError(t, err)
		})
		t.Run("error - embedded key", func(t *testing.T) {
			...
			err := cxt.start()
			require.NoError(t, err)
			_, key, _ := cxt.keyStore.New(audit.TestContext(), crypto.StringNamingFunc("did:nuts:sender#signing-key"), orm.AssertionKeyUsage())
			cxt.network.nodeDID = *nodeDID
...
func TestNetwork_selfTestNutsCommAddress(t *testing.T) {
	t.Run("TLS", func(t *testing.T) {
		...
		// new network
		network := NewNetworkInstance(Config{}, nil, nil, nil, nil, nil)
...
func TestNetwork_validateNodeDID(t *testing.T) {
	...
	t.Run("error - configured node DID does not have key agreement key", func(t *testing.T) {
		...
		cxt.network.strictMode = true
		cxt.network.nodeDID = *nodeDID
		cxt.didStore.EXPECT().Resolve(*nodeDID, nil).Return(&did.Document{}, &resolver.DocumentMetadata{}, nil)
	...
	t.Run("error - configured node DID has key agreement key, but is not present in key store", func(t *testing.T) {
		...
		cxt.network.strictMode = true
		cxt.network.nodeDID = *nodeDID
		cxt.didStore.EXPECT().Resolve(*nodeDID, nil).Return(completeDocument, &resolver.DocumentMetadata{}, nil)
...
func createNetwork(t *testing.T, ctrl *gomock.Controller, cfgFn ...func(config *Config)) *networkTestContext {
	...
	network.serviceResolver = resolver.DIDServiceResolver{Resolver: didStore}
	network.didDocumentFinder = docFinder
	network.state = state
	network.connectionManager = connectionManager
