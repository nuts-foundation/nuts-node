...
func newPeerOmnihashStatistic(input map[transport.PeerID]hash.SHA256Hash) peerOmnihashStatistic {
	var inputCopy = make(map[transport.PeerID]hash.SHA256Hash, len(input))
	for k, v := range input {
		inputCopy[k] = v.Clone()
	}
