...
func Test_defaultMessageSender_broadcastDiagnostics(t *testing.T) {
	...
	sender.broadcastDiagnostics(transport2.Diagnostics{
		Uptime:               1000 * time.Second,
		Peers:                []transport2.PeerID{"foobar"},
		NumberOfTransactions: 5,
		SoftwareVersion:      "1.0",
		SoftwareID:           "Test",
	})
...
func Test_defaultMessageSender_sendTransactionList(t *testing.T) {
	...
	t.Run("ok - paginated", func(t *testing.T) {
		...
		mock.EXPECT().Send(peer, gomock.Any()).DoAndReturn(func(_ transport2.PeerID, msg *transport.NetworkMessage) error {
