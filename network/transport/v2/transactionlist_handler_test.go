...
func TestTransactionListHandler(t *testing.T) {
	...
	t.Run("fn is called", func(t *testing.T) {
		...
		defer cancel()
		value := atomic.Value{}
		handlerFunc := func(peer transport.Peer, envelope *Envelope) error {
			value.Store(true)
			return nil
		}
...
func TestProtocol_handleTransactionList(t *testing.T) {
	...
	t.Run("ok - conversation not marked as done", func(t *testing.T) {
		...
		cStartTime := conversation.lastValidAt.Add(-1 * time.Millisecond)
		conversation.lastValidAt = cStartTime
		mocks.State.EXPECT().Add(context.Background(), tx, payload).Return(nil)
		...
		// timeout is reset
		assert.True(t, conversation.lastValidAt.After(cStartTime))
	})
