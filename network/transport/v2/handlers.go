...
func (p *protocol) handleTransactionPayloadQuery(peer transport.Peer, msg *TransactionPayloadQuery) error {
	...
	tx, err := p.txState.GetTransaction(ctx, hash.FromSlice(msg.TransactionRef))
	if err != nil {
		return err
	}
	...
	data, err := p.txState.ReadPayload(ctx, tx.PayloadHash())
	if err != nil {
		return err
	}
...
func (p protocol) handleTransactionPayload(msg *TransactionPayload) error {
	...
	tx, err := p.txState.GetTransaction(ctx, ref)
	if err != nil {
		return err
	}
	if tx == nil {
	...
	if err = p.txState.WritePayload(ctx, payloadHash, msg.Data); err != nil {
		return err
	}
