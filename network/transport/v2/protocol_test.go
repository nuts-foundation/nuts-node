...
func TestProtocol_gossipTransaction(t *testing.T) {
	t.Run("ok - to gossipManager", func(t *testing.T) {
		...
		proto.gossipTransaction(audit.TestContext(), event)
	})
}
...
func TestProtocol_HandlePrivateTxRetry(t *testing.T) {
	...
	t.Run("errors when reading payload errors", func(t *testing.T) {
		...
		finished, err := proto.handlePrivateTxRetry(audit.TestContext(), event)
	...
	t.Run("errors when decryption fails because the key-agreement key could not be found", func(t *testing.T) {
		...
		mocks.Decrypter.EXPECT().Decrypt(gomock.Any(), keyDID.String(), []byte{1}).Return(nil, crypto.ErrPrivateKeyNotFound)
	...
	t.Run("valid transaction fails when there is no connection available to the node", func(t *testing.T) {
		...
		mocks.Decrypter.EXPECT().Decrypt(gomock.Any(), keyDID.String(), []byte{1}).Return([]byte(peerDID.String()), nil)
	...
	t.Run("valid transaction fails when sending the payload query errors", func(t *testing.T) {
		...
		mocks.Decrypter.EXPECT().Decrypt(audit.ContextWithAuditInfo(), keyDID.String(), []byte{1}).Return([]byte(peerDID.String()), nil)
	...
	t.Run("broadcasts to all participants (except local node)", func(t *testing.T) {
		...
		mocks.Decrypter.EXPECT().Decrypt(gomock.Any(), keyDID.String(), []byte{1}).Return([]byte(encodedPAL), nil)
		// Connection to peer
...
func TestProtocol_decryptPAL(t *testing.T) {
	...
	t.Run("errors when node DID is not set", func(t *testing.T) {
		...
		pal, err := proto.decryptPAL(audit.TestContext(), dummyPAL)
	...
	t.Run("errors when resolving the node DID document fails", func(t *testing.T) {
		...
		_, err := proto.decryptPAL(audit.TestContext(), dummyPAL)
	...
	t.Run("returns nil for empty PAL", func(t *testing.T) {
		...
		pal, err := proto.decryptPAL(audit.TestContext(), [][]byte{})
	...
	t.Run("errors when decryption fails because the key-agreement key could not be found", func(t *testing.T) {
		...
		_, err := proto.decryptPAL(audit.TestContext(), tx.PAL())
	...
	t.Run("valid transaction is decrypted successfully", func(t *testing.T) {
		...
		mocks.Decrypter.EXPECT().Decrypt(gomock.Any(), keyDID.String(), []byte{1}).Return(append(append([]byte(testDID.String()), '\n'), []byte(testDID2.String())...), nil)

		pal, err := proto.decryptPAL(audit.TestContext(), tx.PAL())
