...
func (i *Iblt) Decode() (remaining []hash.SHA256Hash, missing []hash.SHA256Hash, err error) {
	...
	for {
		...
		for bIdx := range i.buckets {
			if (i.buckets[bIdx].count == 1 || i.buckets[bIdx].count == -1) && i.hashKey(i.buckets[bIdx].keySum) == i.buckets[bIdx].hashSum {
				txRef := i.buckets[bIdx].keySum
				if pures[txRef] {
...
func (i *Iblt) bucketIndices(hash uint64) []uint32 {
	...
	for len(indices) < int(i.k) {
		bucketID := next % uint32(numBuckets)
...
func (i *Iblt) MarshalBinary() ([]byte, error) {
	data := make([]byte, numBuckets*bucketBytes)
	for idx := range i.buckets {
...
func (i *Iblt) UnmarshalBinary(data []byte) error {
	...
	d := (*[numBuckets * bucketBytes]byte)(data)
	i.hc = ibltHc
	i.hk = ibltHk
	i.k = ibltK
	for j := 0; j < numBuckets; j++ {
		err := i.buckets[j].UnmarshalBinary(d[j*bucketBytes : (j+1)*bucketBytes])
		if err != nil {
