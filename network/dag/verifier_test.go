...
func TestTransactionSignatureVerifier(t *testing.T) {
	...
	t.Run("referral with key ID", func(t *testing.T) {
		...
		err := NewTransactionSignatureVerifier(&dochelper.StaticKeyResolver{Key: publicKey})(nil, expected)
		assert.NoError(t, err)
	})
	t.Run("wrong key", func(t *testing.T) {
		...
		expected, _ := ParseTransaction(transaction.Data())
		err := NewTransactionSignatureVerifier(&dochelper.StaticKeyResolver{Key: attackerKey.Public()})(nil, expected)
