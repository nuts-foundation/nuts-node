...
type Didman interface {
	...
	// It returns ErrDuplicateService if a service with the given type already exists.
	// It returns docwrapper.DIDServiceQueryError if one of the service references is invalid.
...
type CompoundServiceResolver interface {
	...
	// It returns docwrapper.DIDServiceQueryError if the endpoint doesn't contain a (valid) reference and resolveReferences = true.
